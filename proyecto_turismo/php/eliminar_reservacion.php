...
$sentencia = $base_de_datos->prepare("DELETE FROM reservacion WHERE id = ?;");
$resultado = $sentencia->execute([$id]);

if($resultado === TRUE){
    echo "<script>alert('Se ha eliminado la reservación.');</script>";
    header("refresh:0; url=reservaciones_datos.php");
}
else{
